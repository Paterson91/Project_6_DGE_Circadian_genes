...
heatmap.2(select1_all, col=viridis(256), Colv=TRUE, 
          ...
          main=paste0("Subset ", goi_role, " Clustered"))
dev.copy(png, width = 1000,height = 500, paste0("Subset ", goi_role," - Clustered Heatmap All.png"))
dev.off()
 dev.off()
...
heatmap.2(select1_all, col=viridis(256), Colv=FALSE, 
          ...
          density.info="none", trace="none",margins=c(10,10),
          cexCol=1.3, labRow=row.names(select1_all), labCol = ColourStrategy_all, ColSideColors=cols,
          main=paste0("Subset ", goi_role, " All"))
dev.copy(png, width = 1000,height = 500, paste0("Subset ", goi_role," - Heatmap All.png"))
dev.off()
dev.off()
gc()
