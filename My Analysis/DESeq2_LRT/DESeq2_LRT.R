setwd("/Users/ap14958/OneDrive - University of Bristol/Genomics Facility Bioinformatics/Project #6 Hugh Piggins/My Analysis/DESeq2_LRT/")
...
countData <- as.matrix(read.csv("MidvsEarly.csv", header=TRUE,
                                row.names = 1,
                                as.is=TRUE))
head(countData)
...
metadata = read.csv("SampleMetaData_MidvsEarly.csv", header = TRUE)
...
res=results(dds, contrast = c("Group","Mid_Day",'Early_Day'))

as.data.frame(res[order(res$pvalue),])
