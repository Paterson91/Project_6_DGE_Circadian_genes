...
ColourStrategy2 = goi_lookup$Role

#goi_role = "Glutamatergic Genes"
...
order2 = as.character(goi_lookup$ensembl_gene_id)
...
select1 = dplyr::left_join(rownames_to_column(as.data.frame(iso_table1)), goi_lookup, by =c("rowname"="ensembl_gene_id"))
...
cols2 <- palette(magma(256))[as.fumeric(as.character(ColourStrategy2))]
heatmap.2(select1, col=viridis(256), Colv=FALSE, 
          ...
          cexCol=1.3, labRow=row.names(select1), labCol = ColourStrategy, RowSideColors = cols2, ColSideColors=cols,
          main="All")
